...
https://rawgit.com/HealthCanada/HPFB/master/Structured-Product-Labeling-(SPL)/Style-Sheets/SPM/current/
-->
<xsl:transform version="1.0"
	...
	<xsl:variable name="documentTypes" select="document(concat($oid_loc,$document-id-oid,$file-suffix))"/>
	<xsl:variable name="characteristics" select="document(concat($oid_loc,$product-characteristics-oid,$file-suffix))"/>
...
.
	...
	<xsl:template name="assignedOrganization">
		<xsl:for-each select="/v3:document/v3:author/v3:assignedEntity/v3:representedOrganization/v3:assignedEntity/v3:assignedOrganization/v3:assignedEntity/v3:assignedOrganization">
					<tr><td class="borderCellLeft widthHalf">
					<xsl:value-of select="current()/v3:name"/>
					</td><td class="borderCellLeft">
						<xsl:call-template name="hpfb-label">
